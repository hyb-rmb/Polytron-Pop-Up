...
(lib.FINALBTN = function() {
	this.initialize(img.FINALBTN);
...
(lib.bt_final = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	...
	// Layer_1
	this.instance = new lib.FINALBTN();
	this.instance.setTransform(-110.15,-34,1.2589,1.2589);

	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(0,0,0,0.004)").s().p("EgaLAmWMAAAhMrMA0WAAAMAAABMrg");
	this.shape.setTransform(7.9,91.25);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape},{t:this.instance}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.bt_final, new cjs.Rectangle(-159.6,-154.1,335.1,490.79999999999995), null);
...
if (reversed == null) { reversed = false; }
	...
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {idle:0,tv:1,speaker:2,kulkas:3,ac:4,cuci:5};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);
	...
	this.timeline.addTween(cjs.Tween.get(this.btn_back).wait(1).to({_off:false},0).wait(5));
	...
	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape}]}).to({state:[{t:this.shape_1},{t:this.instance}]},1).to({state:[{t:this.shape_1},{t:this.instance_1}]},1).to({state:[{t:this.shape_1},{t:this.instance_2}]},1).to({state:[{t:this.shape_1},{t:this.instance_3}]},1).to({state:[{t:this.shape_1},{t:this.instance_4}]},1).wait(1));
...
// stage content:
(lib.Polytron_PopUp = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	...
	this.frame_120 = function() {
		this.pop.stop();
		const buttons = [
		    { btn: this.bt_tv, label: "tv" },
		    { btn: this.bt_kulkas, label: "kulkas" },
		    { btn: this.bt_speaker, label: "speaker" },
		    { btn: this.bt_ac, label: "ac" },
		    { btn: this.bt_cuci, label: "cuci" }
		];
		
		// Awal: tampilkan frame idle
		this.currentIndex = -1;
		this.pop.gotoAndStop("idle");
		
		// Sembunyikan bt_final
		this.bt_final.visible = false;
		
		// Array untuk melacak klik
		let clickedButtons = new Array(buttons.length).fill(false);
		
		// Event untuk setiap tombol produk
		buttons.forEach((item, index) => {
		    item.btn.addEventListener("click", () => {
		        this.currentIndex = index;
		        this.pop.gotoAndStop(item.label);
		
		        // Tandai tombol sudah diklik
		        clickedButtons[index] = true;
		
		        // Cek apakah semua sudah diklik
		        if (clickedButtons.every(status => status)) {
		            showFinal();
		        }
		    });
		});
		
		// Tombol close (kembali ke idle)
		this.pop.btn_back.addEventListener("click", () => {
		    this.pop.gotoAndStop("idle");
		});
		
		// Fungsi untuk menampilkan bt_final
		const showFinal = () => {
		    this.bt_final.visible = true;
		
		    // Nonaktifkan tombol produk
		    buttons.forEach(item => {
		        item.btn.mouseEnabled = false;
		        item.btn.alpha = 0.5; // efek disable
		    });
		};
		
		// Event untuk bt_final → buka link di tab baru
		this.bt_final.addEventListener("click", () => {
		    window.open("https://polytron.co.id/", "_blank");
		});
	}
	...
	// Layer_9
	this.bt_final = new lib.bt_final();
	this.bt_final.name = "bt_final";
	this.bt_final.setTransform(155.15,149);
	this.bt_final._off = true;

	this.timeline.addTween(cjs.Tween.get(this.bt_final).wait(120).to({_off:false},0).wait(13));
	...
	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.bt_cuci},{t:this.bt_kulkas},{t:this.bt_tv},{t:this.bt_speaker},{t:this.bt_ac}]},120).wait(13));
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/FINALBTN.png", id:"FINALBTN"},
